...
class ActiveSubstanceTable extends Component {
  state = {
    modelData: [],
    editingKey: '',
    adding: false
  ...
  componentDidMount() {
    fetch(`${BACKEND_URL}/actsubstance/all`, {
      headers: {
      ...
      .then(async (data) => {
        ...
        if (data.error) {
          ...
          this.setState({
            modelData: data.actsubstances.sort((a, b) => a.id - b.id).map(sub => {
              return {
                ...sub,
                key: sub.id
              }
  ...
  deleteHandler = (row) => {
    fetch(`${BACKEND_URL}/actsubstance/${row.id}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${this.props.token}`,
  ...
  save = async (key) => {
    ...
    fetch(`${BACKEND_URL}/actsubstance/edit/${id}`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${this.props.token}`,
  ...
  add = async (values) => {
    ...
    fetch(`${BACKEND_URL}/actsubstance/create/`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${this.props.token}`,
        "Content-Type": "application/json",
      ...
      .then((data) => {
        ...
        if (data.error) {
          ...
          this.setState({
            modelData: [...this.state.modelData, {...data.actsubstance, key: data.actsubstance.id}],
            adding: false
          });
        }
      })
...
export default connect(mapStateToProps)(ActiveSubstanceTable);
